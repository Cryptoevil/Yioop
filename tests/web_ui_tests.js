...
var steps = [
    ...
    /**
     * Test is Sign In link appears on the top of the page.
    ...
    function testSignInLink()
    {
        ...
        if (result.ack) {
            page.evaluate(function()
            {
                ...
                document.querySelector('body div.landing-top-bar' +
                    ' > div.user-nav > ul > li:nth-child(2) > a')
                    .dispatchEvent(ev);
            });
        } else {
    ...
    function testLoginFormExists()
    {
        var result = page.assertExists(
            'body > div > div.landing.non-search > form',
            "Login Form exists", page);
        if (result.ack) {
            var creds = {};
            ...
            page.evaluate(function(creds)
            {
                ...
                document.querySelector(
                    'body > div > div.landing.non-search > form').submit();
                return;
            }, creds);
        } else {
            l("Failed Test");
    ...
    {
        var result = page.assertExists('body > div > div >' +
            ' div.component-container >' +
            ' div:nth-child(3) > ul > li:nth-child(1) > a',
            "Manage groups Link exists", page);
        if (result.ack) {
            page.click('body > div > div > div.component-container > ' +
                ' div:nth-child(3) > ul > li:nth-child(1) > a');
        } else {
            l("Failed Test");
        }
