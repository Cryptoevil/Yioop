...
class SearchController extends Controller implements CrawlConstants
{
    ...
    public function initializeUserAndDefaultActivity(&$data)
    {
        ...
        if (!isset($_REQUEST['a']) || !in_array($_REQUEST['a'],
            ...
            } else if ($activity != "more" && $activity != "trending"){
                $activity = "query";
            }
        }
        ...
        if ($activity == "more") {
            ...
            if ($arg == 'chart' && !empty($_REQUEST['term']) &&
                ...
                $term = $this->clean($_REQUEST['term'], 'string');
                $periods = [C\ONE_DAY, C\ONE_WEEK];
                $period = in_array($_REQUEST['period'], $periods) ?
                    $_REQUEST['period'] : C\ONE_DAY;
                $data['PERIOD'] = $period;
                $data['TERM'] = $term;
                $data['CHART_DATA'] = $trending_model->termScoresForPeriod(
                    $term, $period, L\getLocaleTag());
                if ($_SERVER["MOBILE"]) {
                    $properties = ["title" => "",
        ...
        switch ($activity) {
            case "related":
                ...
                $phrase_results = $phrase_model->getPhrasePageResults(
                    ...
                    $guess_semantics, $save_timestamp, $limit_feeds);
    ...
    public function queryRequest($query, $results_per_page, $limit = 0,
        $grouping = 0, $save_timestamp = 0, $limit_feeds = true)
    {
        if (!C\API_ACCESS) {
            return null;
        }
        ...
        $this->processQuery($data, $query, "query", "", $results_per_page,
                $limit, 0, $grouping, $save_timestamp, $limit_feeds);
        return $data;
    }
    /**
        ...
        $limit_feeds = true)
    {
        ...
        $this->processQuery($data, "", "related", $url, $results_per_page,
            $limit, $crawl_time, $grouping, $save_timestamp, $limit_feeds);
        return $data;
    }
    /**
     * Part of Yioop Search API. Performs a related to a given url
