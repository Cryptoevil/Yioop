...
class MoreoptionsElement extends Element
{
    ...
    public function render($data)
    {
        ...
        <tr><td><ul class='square-list'><li><a href="<?=$settings_url .
                ...
                ?>"><?= tl('moreoptions_element_settings') ?></a></li>
            <?php
            if (!$_SERVER["MOBILE"]) { ?>
                </ul></td>
            ...
            if (!$logged_in) {
                ...
                tl('moreoptions_element_signin') ?></a></li><?php
            } else {
                ?><li><a href="<?=$admin_url . $token_query ?>"><?=
                tl('moreoptions_element_admin') ?></a></li><?php
            }
            if (!$_SERVER["MOBILE"]) {
                e('</ul></td>');
            }
            ...
            if ((!$logged_in) &&
                ...
                if (!$_SERVER["MOBILE"]){ e("<td><ul  class='square-list'>"); } ?>
                <li><a href="<?=rtrim($register_url . "a=createAccount&amp;" .
                    $token_query, "?&amp;") ?>"><?=
                    tl('moreoptions_element_create_account')
                    ?></a></li>
                </ul></td>
                <?php
            }
        ...
        $tools[$trending_url] = tl('moreoptions_element_trending');
        if ($tools != []) {
            $max_column_num = 10;
            if ($_SERVER["MOBILE"]) {
                $num_columns = 1;
