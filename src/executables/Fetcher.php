...
class Fetcher implements CrawlConstants
{
    ...
    /**
     * Adds thumbs for websites with a self::THUMB_URL field bby downloading the
     *  linked to images and making a thumb from it.
     *
     * @param array& $sites associative array of web sites information to add thumbs
     *  for. At least one site in the array should have a self::THUMB_URL
     *  field that we want have the thumb of
     */
    ...
    public function buildMiniInvertedIndex()
    {
        ...
        */
        ...
        for ($i = 0; $i < $num_seen; $i++) {
            ...
            if (!isset($site[self::HASH]) || (isset($site[self::ROBOT_METAS]) &&
                in_array("JUSTFOLLOW", $site[self::ROBOT_METAS]))) {
                continue;
            }
            $doc_rank = false;
            ...
            if (isset($site[self::TYPE]) && $site[self::TYPE] == "link") {
                ...
                $meta_ids = PhraseParser::calculateLinkMetas($site_url,
                    $host, $site[self::DESCRIPTION], $link_origin, $url_info);
            } else {
                $is_link = false;
            ...
            /*
                ...
                in what indexing, so don't index again. $site[self::JUST_METAS]
            ...
            if (!$this->no_process_links && !isset($site[self::JUST_METAS]) &&
                ...
                foreach ($site[self::LINKS] as $url => $url_info) {
                    ...
                    $useful_text = preg_replace("/\.\.|\s/u", "", $useful_text);
                    ...
                    $summary[self::ENCODING] = (empty($site[self::ENCODING])) ?
                    ...
                    $link_meta_ids = PhraseParser::calculateLinkMetas($url,
                        $link_host, $link_text, $site_url, $url_info);
                    if (!isset($this->found_sites[self::INVERTED_INDEX][
                        $part_num])) {
