...
class IndexShard extends PersistentStructure implements CrawlConstants
{
        ...
        foreach ($word_lists as $word => $position_list) {
            if (isset($position_list["cond_max"])) { //for now
                $word_id = crawlHashPath($word, $position_list["cond_max"],
                    true);
                /* cond_max corresponds to a phrase in the document
                   as each word will also be stored in addition to
                   the phrase, and as we are using $occurences to
                   count the number of words in th document, we don't
                   want to double count so we set to zero for this case
                 */
                $occurrences = 0;
     ...
     */
    public function makeItem($posting, $num_doc_or_links)
    {
