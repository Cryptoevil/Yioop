...
class WikiParser implements CrawlConstants
{
        ...
        $minimal = false)
    {
        ...
        //assume substitutions are applied after html entities called on string
        $substitutions = [
            ['/(\A|\n)=\s*([^=]+)\s*=/', "\n<h1 id='$2'>$2</h1>"],
            ['/(\A|\n)==\s*([^=]+)\s*==/', "\n<h2 id='$2'>$2</h2>"],
            ['/(\A|\n)===\s*([^=]+)\s*===/', "\n<h3 id='$2'>$2</h3>"],
        ...
        $minimal_substitutions = array_merge($minimal_substitutions,
            $link_substitutions);
        $substitutions = array_merge($minimal_substitutions, $substitutions);
        $this->matches = [];
        $this->replaces = [];
        $this->braces_matches = [];
        ...
        $handle_big_files = false)
    {
        ...
        $document = preg_replace_callback(
            "/\<pre\>(.+?)\<\/pre\>/s",
            C\NS_LIB . "spaceDecodeCallback", $document);
        $document = preg_replace_callback(
            "/&lt;nowiki&gt;(.+?)&lt;\/nowiki&gt;/s",
    ...
    public function processProvidedRegexes($matches, $replaces, $document)
    {
        if (strlen($document) < C\MAX_GROUP_PAGE_LEN) {
            ...
            for ($i = 0; $i < $num_matches; $i++) {
                ...
                $document = preg_replace($matches[$i], $replaces[$i],
                    $document);
            }
        }
        return $document;
    ...
    function fetchLinks($document)
    {
        ...
        foreach ($link_substitutions as $key_link => $value_link) {
            $link_substitution = $link_substitutions[$key_link];
            preg_match_all($link_substitution, $document, $matches,
                PREG_PATTERN_ORDER);
        ...
        foreach ($links as $key => $link) {
            ...
            // get rid of link fragments
            list($extract, ) = explode('#', $internal_link, 2);
...
/**
 * Callback used to encode the contents of pre tags so they
...
/**
 * Callback used to encode the contents of span tags so they
 * newlines within them don't accidentally get treated as new wiki paragraphs
 *
 * @param array $matches $matches[1] should contain the contents of a span tag
 * @return string encoded contents surrounded by an escaped pre tag.
 */
 ...
 * @param array $matches $matches[1] should contain the contents of a pre tag
 * @return string cleaned contents surrounded by a pre-formatted tag.
 */
function spaceDecodeCallback($matches)
{
    $out = strip_tags($matches[1], "<b><i><br><span><a><u><s><hr>");
    return "<pre>" . $out . "</pre>";
}
