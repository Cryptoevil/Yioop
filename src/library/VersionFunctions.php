...
function upgradeDatabaseVersion20(&$db)
{
    ...
    <script src="<?= C\BASE_URL ?>/scripts/basic.js" ></script>
    <script>
    <?php
...
{
    $db->execute("CREATE TABLE AVERAGE_RATING (AVG_RATING INT)");
    $db->execute("INSERT INTO AVERAGE_RATING VALUES (0)");
    $db->execute("CREATE TABLE IDF (WORD_ID INT, ITEM_IMP FLOAT,
        USER_IMP FLOAT)");
    $db->execute("CREATE TABLE IDF_TEMP (WORD_ID INT, ITEM_IMP
        FLOAT, USER_IMP FLOAT)");
    $db->execute("CREATE TABLE ITEM_BIAS(ITEM_ID INT,
        ITEM_BIAS FLOAT)");
    $db->execute("CREATE TABLE ITEM_WORD_FREQUENCY (ITEM_ID INT,
        WORD_ID INT, FREQUENCY INT, LOGFREQUENCY FLOAT)");
    $db->execute("CREATE TABLE ITEM_WORD_WEIGHTS(WORD_ID INT,
        ITEM_ID INT ,WEIGHT FLOAT)");
    $db->execute("CREATE TABLE LIVE_TABLE (TRENDING INT,
        THREADS INT, GROUPS INT)");
    $db->execute("INSERT INTO LIVE_TABLE VALUES (1,1,1)");
    $db->execute("CREATE TABLE PREDICTED_TABLE(USER_ID INT,
        ITEM_ID INT, USER_BIAS FLOAT, ITEM_BIAS FLOAT, RATING FLOAT)");
    $db->execute("CREATE TABLE RECOMMENDATION_LIST_GROUPS_1
        (USER_ID INT, GROUP_1 INT, GROUP_2
        INT, GROUP_3 INT, RECOMMEND_TYPE VARCHAR)");
    $db->execute("CREATE TABLE RECOMMENDATION_LIST_GROUPS_2
        (USER_ID INT, GROUP_1 INT, GROUP_2
        INT, GROUP_3 INT, RECOMMEND_TYPE VARCHAR)");
    $db->execute("CREATE TABLE RECOMMENDATION_LIST_THREADS_1
        (USER_ID INT, GROUP_1 INT, GROUP_2 INT, GROUP_3 INT,
...
function upgradeDatabaseVersion54(&$db)
{
    ...
    $new_tables_and_indexes_sql = [
        ...
        "CREATE TABLE USER_ITEM_SIMILARITY (USER_ID INTEGER, THREAD_ID INTEGER,
            SIMILARITY FLOAT, GROUP_MEMBER INTEGER,
...
/**
 * Upgrades a Version 63 version of the Yioop database to a Version 62 version
 * @param object $db datasource to use to upgrade.
 */
function upgradeDatabaseVersion64(&$db)
{
    ...
    $db->execute("CREATE TABLE ITEM_IMPRESSION_STAT(ITEM_ID INTEGER,
        ITEM_TYPE INTEGER, UPDATE_PERIOD INTEGER, NUM_VIEWS INTEGER DEFAULT -1,
        FUZZY_NUM_VIEWS INTEGER DEFAULT -1)");
}
