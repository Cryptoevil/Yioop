...
class PhraseParser
{
    ...
    /**
     * Given an array of terms, an index, and a threshold, tries to find a
     * splitting of the terms into phrases which will still return at least
     * threshold many results. As phrases are looked up as a unit this
     * will save on the number of conjuncts in any conjunctive queries we do.
     * For example, it might take in the array ["San", "Jose", "gymnist"].
     * If the whole phrase "San Jose gymnist" occurs more than threshold times,
     * then ["San Jose gymnist"] would be returned. Otherwise, it recurses
     * and chooses the splitting with more results amongst
     * extractTermsWholePhrase on  ["San", "Jose"] and ["Jose", "gymnist"]
     * and returns the array with this together with the left out term.
     *
     * @param array $terms to try to group into phrases
     * @param string $index_name index to see how frequent terms appear in
     * @param int $threshold minimum number of results before a phrase is
     *      deeemed to have enough reesults
     * @param bool $with_pseudo_count used as part of recursive calling when
     *      this method calls itself on subarrays to signal if a count of
     *      approximate number of results is desired.
     * @return array a splitting of terms into an array of phrases and terms
     */
    public static function extractTermsWholePhrase($terms, $index_name,
        $threshold, $with_pseudo_count = false)
    {
        $num_terms = count($terms);
        if (($num_terms <= 1 || C\SUFFIX_PHRASES != 'true') &&
            !$with_pseudo_count) {
            return $terms;
        }
        $whole_phrase = implode(" ", $terms);
        $num_whole_phrase_results = IndexManager::numDocsTerm($whole_phrase,
            $index_name, $threshold);
        if (($num_terms <= 1 || C\SUFFIX_PHRASES != 'true') &&
            $with_pseudo_count) {
            return [$terms, $num_whole_phrase_results];
        }
        /*
           If have more than $threshold (default 10, one page) worth of
           whole phrase results then use whole phrase for results
         */
        if ($num_whole_phrase_results >= $threshold) {
            if ($with_pseudo_count) {
                return [[$whole_phrase], $num_whole_phrase_results];
            }
            return [$whole_phrase];
        } else if ($num_terms < C\PHRASE_THRESHOLD) {
            if ($with_pseudo_count) {
                // as a conjunctive query will get at least this many results
                return [$terms, $num_whole_phrase_results];
            }
            return $terms;
        } else {
            /* have at least three terms in this case where
               the whole phrase doesn't have $threshold results
             */
            $original_terms = $terms;
            $first_term = array_shift($terms);
            //see terms generated if removed first term from list
            list($rest_terms, $num_rest_terms_results) =
                self::extractTermsWholePhrase($terms,
                $index_name, $threshold, true);
            array_unshift($rest_terms, $first_term);
            $last_term = array_pop($original_terms);
            list($first_terms, $num_first_terms_results)  =
                self::extractTermsWholePhrase($original_terms,
                $index_name, $threshold, true);
            $first_terms[] = $last_term;
            $max_results = max($num_rest_terms_results,
                $num_first_terms_results);
            $max_terms = ($num_rest_terms_results == $max_results) ?
                $rest_terms : $first_terms;
            /*  Next check is to see if single query doesn't lose too many
                results. If so, since it is faster, single query is faster
                use it.
             */
            if (2 * $num_whole_phrase_results > $max_results) {
                if ($with_pseudo_count) {
                    return [[$whole_phrase], $num_whole_phrase_results];
                }
                return [$whole_phrase];
    ...
    public static function extractPhrasesInLists($string, $lang = null)
    {
        ...
        if (empty($phrase_and_sentences["TERMS_AND_PHRASES"])) {
    ...
    {
        $string = mb_ereg_replace("\s*\&apos\;\s*", "'", $string);
        ...
        if (!empty($site[CrawlConstants::IS_VIDEO])) {
            ...
            if (!empty($site[CrawlConstants::DURATION])) {
                $durations = [ 60 => "one-minute", 300 => "five-minute",
                    ...
                    1800 => 'half-hour', 3600 => 'one-hour', 7200 => 'two-hour'
                ];
