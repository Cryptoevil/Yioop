...
EOT;
    ...
    public function crawlStalled($machine_urls = null)
    {
        ...
        $stat_files = glob("$stat_suffix*$stat_suffix");
        $statuses = [];
        foreach ($stat_files as $stat_file) {
            ...
            if ($non_repeating && filemtime($stat_file) +
                ...
                $schedule_stat_file = C\CRAWL_DIR .
                    "/schedules/$channel-schedule_status.txt";
                $schedule_status_exists = file_exists($schedule_stat_file);
                if ($schedule_status_exists && $non_repeating &&
                    filemtime($schedule_stat_file) + C\CRAWL_TIME_OUT < time()) {
                    $statuses[$channel] = true;
                } else {
